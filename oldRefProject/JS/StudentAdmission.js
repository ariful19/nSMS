...
function PopulateDistrict() {
    if ($("#ddlPresentDiv :selected").val() == "0") {
        ...
        $.ajax({
            ...
            dataType: "json",
            url: "AdmissionForm.aspx/LoadDistrictByDivision",
...
function PopulateThana() {
    if ($("#ddlPresentDis :selected").val() == "0") {
        ...
        $.ajax({
            ...
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            url: "AdmissionForm.aspx/LoadThanaByDistrict",
...
$("#ddlInstiteKnown").on('change', function () {
    if ($("#ddlInstiteKnown :selected").val() == "1") {
        
        $("#txtInsKnownName").attr('disabled', false);
        $("#txtInsKnownDesignation").attr('disabled', false);
    }
    else {
        $("#txtInsKnownName").attr("disabled", true);
        $("#txtInsKnownDesignation").attr("disabled", true);
    }
});

$("#ddlGameAvoid").on('change', function () {
    if ($("#ddlGameAvoid :selected").val() == "1") {
       
        $("#txtGameAvoidDetail").attr('disabled', false);
        
    }
    else {
        $("#txtGameAvoidDetail").attr("disabled", true);
    
    }
});

$("#ddlAlergyProb").on('change', function () {
    if ($("#ddlAlergyProb :selected").val() == "1") {
       
        $("#txtAvoidable").attr('disabled', false);
        
    }
    else {
        $("#txtAvoidable").attr("disabled", true);
        
    }
});

$("#ddlPhysicalDrawback").on('change', function () {
    if ($("#ddlPhysicalDrawback :selected").val() == "1") {

        $("#txtDrawbackDetails").attr('disabled', false);

    }
    else {
        $("#txtDrawbackDetails").attr("disabled", true);
...
//    $(document).on("click","#ddlInstiteKnown", function(){
//        var check_vel= $(this).is("#ddlInstiteKnown").value;
//        if(check_vel == "1"){
...
function load() {
    ...
    $('#pnlPreviousStudy').hide();
    $('#chkPreviousStudy').click(function () {
        if (this.checked) {
            $('#pnlPreviousStudy').slideDown();
        }
        else {
            $('#pnlPreviousStudy').slideUp();
        }
    });

    $('#pnlReference').hide();
    $('#chkReference').click(function () {
        if (this.checked) {
            $('#pnlReference').slideDown();
        }
        else {
            $('#pnlReference').slideUp();
        }
    });

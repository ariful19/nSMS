 ...
 protected string GetCriteriass()
    ...
    protected void btnSearch_Click(object sender, EventArgs e)
    {
        ...
        if (presentDt.Rows.Count > 0)
        {
            ...
            if (diffTime > 45)
            {
                rptStudentRoll.DataSource = presentDt;
                rptStudentRoll.DataBind();
            ...
            {
                rptStudentRoll.DataSource = null;
                rptStudentRoll.DataBind();
                btnSave.Visible = false;
                chkSMS.Checked = false;
    ...
    private void IsHasMobileNo(DataTable dt)
    {
        string errorMgs = "";
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        if (rdList.SelectedValue != "")
        {
            if (finalDt.Rows.Count > 0)
            {
                ...
                if (diffTime > 45)
                {
                    foreach (RepeaterItem item in rptStudentRoll.Items)
                    {
            ...
            else
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    obj.AttendenceInsert(dt, year, month, Page.User.Identity.Name, date);
                    if (Validation())
                    {                
                        SendSms(dt);                        
                    }
        ...
        for (int i = 0; dt.Rows.Count > i; i++)
        {
            ...
            if (!String.IsNullOrEmpty(mobile))
            {
                if (dts.Rows.Count > 0)
                {
                    ...
                    if (msgCredit.AvailableBalance > (decimal)0.39)
                    {
                        ...
                        if (messageBody != "")
                        {
                            ...
                            msgCredit.AvailableBalance -= (decimal)0.40;
